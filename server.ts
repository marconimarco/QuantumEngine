  ...
  app.post("/api/quantum-bi/chat", async (req, res) => {
    try {
      ...
      const patchedSystemPrompt = `${systemPrompt || ""}\n\n` + 
        `=== MANDATORY ARCHITECTURAL DIRECTIVES FOR QUANTUM CODE GENERATION (MUST COMPLY STRICTLY) ===\n` +
        `0. LANGUAGE REQUIREMENT: ALWAYS RESPOND ENTIRELY IN ENGLISH. All conversational text, explanations, questions, and guides must be written in clear English.\n` +
        `1. OPENQASM INTEGRITY REQUIREMENT (TRANSLATION BUG FIX):\n` +
        `When generating OpenQASM 2.0 code (via qasm2_dumps or static text), the size of the register 'qreg q[N]' must match EXACTLY the number of qubits actually initialized and manipulated in the circuit.\n` +
        `It is strictly forbidden to declare a large register (e.g. qreg q[28]) and apply RY or CRY gates only to the first and last qubits. Every qubit declared in the register must receive its corresponding RY rotation based on data mapping in radians.\n\n` +
        `2. COMPARATOR DISTRIBUTION LOGIC (CRY):\n` +
        `In the scenario with Comparator Qubit (Target Ancilla at index N), the controlled phase shift gate 'cry' must be applied to ALL qubits of the register, from q[0] up to q[N-1]. The applied rotation angle must be uniformly distributed (Threshold_Angle / N) on each asset qubit to ensure correct energy accumulation toward the target.\n\n` +
        `3. HOMOGENEITY BETWEEN PYTHON AND OPENQASM:\n` +
        `The Python code (standard Qiskit 1.x) and the OpenQASM output must be an exact mirror of each other. If angles for 3 assets (e.g. Gold, Platinum, Oil) across multiple columns are calculated and declared in Python, the OpenQASM output must explicitly contain all 'ry' and 'cry' instructions for each asset. Code must never stop or be truncated.\n\n` +
        `4. MATHEMATICAL CRASH PROTECTION (DATA CLIPPING):\n` +
        `In all generated Python scripts, before calculating the angle in radians with '2 * np.arcsin(np.sqrt(P))', always apply safety clipping via NumPy to prevent NaN values generated by high stress multipliers:\n` +
        `P_clipped = np.clip(P, 0, 1)\n` +
        `theta = 2 * np.arcsin(np.sqrt(P_clipped))\n\n` +
        `5. INVIOLABLE QISKIT 1.x STANDARD:\n` +
        `Never use the deprecated 'execute()' function. Use exclusively modern Primitives for result extraction:\n` +
        `sampler = StatevectorSampler()\n` +
        `job = sampler.run([qc], shots=1000)\n` +
        `counts = job.result().data.get_counts(qc)\n\n` +
        `--- INTERNAL CHECK STRUCTURE BEFORE RESPONDING ---\n` +
        `Before outputting, perform a mental check: "Have I applied RY and CRY gates to all declared qubits in the QASM? Is there entanglement between heterogeneous sources as required by the advanced mode? Is my response strictly in English?". If not, regenerate before showing to the user.`;
